...
export function AiSignalPanel({ data, className }: AiSignalPanelProps) {
	...
	return (
		...
		>
			...
			<h2 className='text-sm font-semibold uppercase tracking-[0.35em] text-accent-teal'>
				{data.headline}
			</h2>
			...
			<ul className='mt-6 space-y-3 text-xs text-dawn/60'>
				{data.ideas.map((idea) => (
					...
					>
						<span
							className={`absolute -left-3 top-1/2 h-6 w-6 -translate-y-1/2 rounded-full bg-gradient-to-br ${accentRing}`}
						/>
