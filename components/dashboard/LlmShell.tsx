  ...
  return (
    <DashboardLayout hero={llmCopy.hero}>
      <section className="rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {llmCopy.modes.map((mode) => (
            <article key={mode.id} className="relative overflow-hidden rounded-[24px] border border-white/10 bg-white/5 p-4">
              <div className={`absolute inset-0 bg-gradient-to-br ${mode.accent} opacity-40`} />
      ...
      <section className="rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur-2xl">
        ...
        <h2 className="mt-4 text-xl font-semibold text-white">{llmCopy.context.title}</h2>
