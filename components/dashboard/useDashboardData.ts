...
function mapAssistantModesToPrompts(
	response: AssistantModesResponse | undefined,
): AiPrompts {
...
export function useDashboardData() {
	...
	const assistantQuery = useQuery({
		...
		queryFn: () =>
			fetchJson<AssistantModesResponse>('/api/assistant/modes'),
		staleTime: 300_000,
	});
	...
	const aiPrompts = useMemo(
		() => mapAssistantModesToPrompts(assistantQuery.data),
		[assistantQuery.data],
	);

	const trendTopics = useMemo<TrendTopic[]>(
		() => mapPostsToTopics((posts as any) || []),
		[posts],
	);
