		...
		<div className='rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur-2xl'>
			<div className='grid gap-8'>
				<header className='flex flex-col gap-2 text-white'>
					<p className='text-xs uppercase tracking-[0.3em] text-dawn/60'>
						ИИ-проводник
					</p>
					<h2 className='font-display text-2xl'>
						Создаём общую фабрику знаний
					</h2>
				</header>
				<section className='grid gap-6 lg:grid-cols-[2fr_1fr]'>
					<div className='flex flex-col gap-4 rounded-[24px] border border-white/10 bg-white/5 p-4'>
									...
									{message.sources && (
										<div className='mt-3 space-y-3'>
											{message.sources.map((source) => (
												<div
													key={source.title}
													className='rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-dawn/60'
												>
						...
						<form
							...
							className='relative mt-2 flex items-center rounded-full border border-white/10 bg-white/5 pr-2'
						>
							<input
								name='prompt'
					...
					<aside className='flex flex-col gap-4 rounded-[24px] border border-white/10 bg-white/5 p-4'>
						...
						<ul className='space-y-3'>
							{sources.map((source) => (
								<li
									key={source.title}
									className='rounded-2xl bg-white/5 p-3 text-sm text-dawn/70'
								>
									<p className='font-medium text-white'>
										{source.title}
									</p>
