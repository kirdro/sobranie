...
import type { Notification, NotificationsFilter, ApiResponse } from '../types';
...
export const markNotificationReadFx = createEffect<string, void>(
	async (notificationId) => {
		...
		if (!response.ok) {
			throw new Error(
				`Mark notification read failed: ${response.statusText}`,
			);
		}
	},
);

export const markAllNotificationsReadFx = createEffect<void, void>(async () => {
	const token = getAuthToken();
	...
	if (!response.ok) {
		throw new Error(
			`Mark all notifications read failed: ${response.statusText}`,
		);
	}
});

export const clearNotificationsFx = createEffect<void, void>(async () => {
	const token = getAuthToken();
