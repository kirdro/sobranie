...
export async function GET(request: Request) {
  ...
  // Check cache first
  if (postsCache && postsCache.params === paramsString && Date.now() - postsCache.timestamp < POSTS_CACHE_TTL) {
  ...
  try {
    ...
    // Cache the result
    postsCache = { data: posts, params: paramsString, timestamp: Date.now() };
